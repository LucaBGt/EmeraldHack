...
{
    u8 spriteId;
    u16 sheet = LoadSpriteSheet(&sSpriteSheet_MoveEffectMons[a3]);
    u16 palette = AllocSpritePalette(sSpriteTemplate_MoveEffectMons[a3].paletteTag);
    ...
    if (!isBackpic)
        spriteId = CreateSprite(&sSpriteTemplate_MoveEffectMons[a3], x, y + gMonFrontPicCoords[species].y_offset, subpriority);
    else
        spriteId = CreateSprite(&sSpriteTemplate_MoveEffectMons[a3], x, y + gMonBackPicCoords[species].y_offset, subpriority);
